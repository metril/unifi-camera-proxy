...
client.on('connect', () => {
    ...
    // Subscribe to Frigate reviews topic
    client.subscribe(config.frigateReviewsTopic, (err) => {
        if (err) {
            console.error('Subscription error:', err);
            logEvent(`ERROR: Failed to subscribe to ${config.frigateReviewsTopic}`);
        } else {
            console.log(`Subscribed to: ${config.frigateReviewsTopic}`);
            logEvent(`Subscribed to: ${config.frigateReviewsTopic}`);
        }
    });
});
...
console.log(`Logging reviews to: ${logPath}`);
