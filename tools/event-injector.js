...
// Configuration
const DEFAULT_VIEWPORT_WIDTH = 2560;
const DEFAULT_VIEWPORT_HEIGHT = 1920;
const ACTUAL_VIEWPORT_WIDTH = 2560;
const ACTUAL_VIEWPORT_HEIGHT = 1440;
const BOX_VIEWPORT_WIDTH = 1024;
const BOX_VIEWPORT_HEIGHT = 1024;
const BOX_SIZE = 50;

// Position definitions (where to place the box in actual viewport coordinates)
const POSITIONS = {
    topLeft: { x: 0, y: 125 },
    topRight: { x: 960, y: 125 },
    bottomLeft: { x: 0, y: 824 },
    bottomRight: { x: 960, y: 824 },
    center: { x: 487, y: 487 }
};

// Convert box viewport coordinates to actual viewport coordinates
function boxToActualViewport(boxX, boxY) {
    // Based on known mappings:
    // topLeft: (0, 0) -> (0, 125)
    // center: (487, 487) -> (487, 487)
    // bottomRight: (974, 974) -> (960, 758) for top-left of 50x50 box

    // Y-axis: maps 1024 height to 633 pixels (125 to 758)
    // Scale: 633 / 1024 = 0.61816
    const yScale = 633 / 1024;
    const yOffset = 125;

    // X-axis: maps 1024 width to 960 pixels
    // Scale: 960 / 974 = 0.98563
    const xScale = 960 / 974;
    const xOffset = 0;

    return {
        x: boxX * xScale + xOffset,
        y: boxY * yScale + yOffset
    };
}
...
function calculateBox(position) {
    ...
    // Positions are already in actual viewport coordinates
    const x1 = pos.x;
    const y1 = pos.y;
...
function calculateRegion(box) {
    ...
    return [
        ...
        Math.min(ACTUAL_VIEWPORT_WIDTH, box[2] + margin),
        Math.min(ACTUAL_VIEWPORT_HEIGHT, box[3] + margin)
    ];
}

// Calculate normalized path data based on box position
function calculatePathData(box, baseTime) {
    const centerX = (box[0] + box[2]) / 2 / ACTUAL_VIEWPORT_WIDTH;
    const centerY = (box[1] + box[3]) / 2 / ACTUAL_VIEWPORT_HEIGHT;
...
async function main() {
    ...
    if (positions.length === 0) {
        console.log('Usage: node frigate-injector.js <position1> [position2] ...');
        console.log(`Valid positions: ${Object.keys(POSITIONS).join(', ')}`);
        console.log('\nExample: node frigate-injector.js topRight bottomRight');
    ...
    console.log('=== Frigate MQTT Event Injector ===');
    console.log(`Default viewport: ${DEFAULT_VIEWPORT_WIDTH}x${DEFAULT_VIEWPORT_HEIGHT}`);
    console.log(`Actual viewport: ${ACTUAL_VIEWPORT_WIDTH}x${ACTUAL_VIEWPORT_HEIGHT}`);
    console.log(`Box viewport: ${BOX_VIEWPORT_WIDTH}x${BOX_VIEWPORT_HEIGHT}`);
    console.log(`Box size: ${BOX_SIZE}x${BOX_SIZE} pixels`);
